...
function getNextFriday(date) {
  ...
  let restDays = 5 - currentDay;

  if (restDays <= 0) {
    restDays += 7;
  }

  days.setDate(days.getDate() + restDays);
...
function formatDate(date) {
  ...
  return `${month}/${day}/${year}, ${formattedHours}:${min}:${sec} ${ampm}`;
}
...
function getNextFridayThe13th(date) {
  ...
  let currMoth = startDay.getUTCMonth();
  let currYear = startDay.getUTCFullYear();
  let isFound = false;

  while (!isFound) {
    const a = new Date(currYear, currMoth, 13);
    const weekDay = a.getUTCDay() + 1;
  ...
  return new Date(currYear, currMoth, 13);
}
...
function isLeapYear(date) {
  if (date.getUTCFullYear() % 100 === 0 && date.getUTCFullYear() % 400 === 0) {
    return true;
  }

  return date.getUTCFullYear() % 4 === 0 && date.getUTCFullYear() % 100 !== 0;
}
